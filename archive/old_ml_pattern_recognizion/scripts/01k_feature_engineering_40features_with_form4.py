...
def calculate_technical_features(df: pd.DataFrame, stock_id: int) -> pd.DataFrame:
    ...
    ti = TechnicalIndicators(stock_id)
    ...
    df['atr'] = ti.add_atr(df, period=14)
    df['daily_range'] = (df['high'] - df['low']) / df['close']

    # Core Volume (3)
    df['log_volume'] = np.log(df['volume'])
    df['obv'] = ti.add_obv(df)
    ...
    # ADX (2)
    adx_result = ti.add_adx(df, period=14)
    ...
    # Other indicators
    df['psar'] = ti.add_parabolic_sar(df)
    df['cci'] = ti.add_commodity_channel_index(df)
    df['roc'] = ti.add_rate_of_change(df, period=20)

    aroon_result = ti.add_aroon_oscillator(df)
    if isinstance(aroon_result, pd.DataFrame) and not aroon_result.empty:
        df['aroon_osc'] = aroon_result['Aroon_Osc']
    else:
        df['aroon_osc'] = 0

    df['linearreg_slope'] = ti.add_linear_regression_slope(df, period=14)
    df['gap'] = (df['open'] - df['close'].shift(1)) / df['close'].shift(1)
    df['natr'] = df['atr'] / df['close'] * 100
    df['mfi'] = ti.add_money_flow_index(df)

    stoch_result = ti.add_stochastic(df)
    if isinstance(stoch_result, pd.DataFrame) and not stoch_result.empty:
