#!/usr/bin/env python3
"""
Feature Engineering: Clean Feature Set
Removes redundant and non-numeric features for ML training
"""
...
def filter_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Remove redundant and non-ML-friendly features

    Removes:
    - All _reason columns (text descriptions)
    - All text-based _signal columns (except numerical macd_signal)
    - Redundant price derivatives
    - SMA-200 (high NaN percentage)

    Keeps:
    - Insider trading features (ceo_bought_30d, cto_bought_30d, cfo_bought_30d, cluster_buying_30d)
    - Numerical indicators
    - Numerical macd_signal (EMA of MACD, not the text classification)
    """
    print("Filtering features...")

    # Columns to remove patterns
    remove_patterns = [
        '_reason',              # All text reason columns
    ]

    # Text-based signal columns to remove (numerical macd_signal is kept)
    text_signals = [
        'rsi_signal', 'bb_signal', 'ma_signal', 'adx_signal',
        'psar_signal', 'stoch_signal', 'stochrsi_signal',
        'atr_signal', 'natr_signal', 'ultosc_signal',
        'aroon_signal', 'cci_signal', 'mfi_signal',
        'obv_signal', 'ppo_signal', 'roc_signal',
        'willr_signal', 'bop_signal', 'cmo_signal',
        'trix_signal', 't3_signal', 'vwap_signal',
        'linearreg_signal', 'ht_signal', 'ht_trendmode_signal',
        'ad_signal', 'adosc_signal', 'apo_signal',
        'kama_signal', 'mama_signal', 'tema_signal',
        'stddev_signal', 'adx_reason', 'adx_signal',  # text-based
        'kc_signal', 'kc_reason',
    ]

    # Redundant price derivatives
    price_derivatives = [
        'high_close', 'high_diff', 'high_low',
        'low_close', 'low_diff'
    ]
    ...
    for pattern in remove_patterns:
        cols_to_drop.update([col for col in df.columns if pattern in col.lower()])

    # Add explicit lists
    cols_to_drop.update(text_signals)
    cols_to_drop.update(price_derivatives)
    cols_to_drop.update(high_nan)
    ...
    print(f"  Removed {original_count - final_count} columns ({len(cols_to_drop)} found)")
    print(f"  Remaining features: {final_count}")

    # List what was removed
    removed = [col for col in cols_to_drop if col in df.columns]
    if removed:
        print(f"\n  Removed columns:")
...
def main():
    print("=" * 70)
    print("Feature Engineering: Clean Feature Set")
    print("=" * 70)
    ...
    if all_data:
        ...
        print(f"\nBefore filtering: {len(final_df.columns)} columns")

        # Filter features - remove redundant columns
