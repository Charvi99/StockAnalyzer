#!/usr/bin/env python3
"""
Fix News Features - Option 2: Fill with Neutral (No Data Leakage)

This script:
1. Drops the 20 incorrectly created news columns
2. Queries daily-aggregated news from database (proper aggregation)
3. Merges on date only (not exact timestamp match)
4. Fills missing news with 0 (neutral sentiment, no leakage)

Usage:
    python scripts/fix_news_features_v2.py
"""
...
print("="*80)
print("NEWS FEATURE FIX - OPTION 2")
print("="*80)
print()
print("Strategy: Fill with 0 (neutral sentiment)")
print("- No data leakage from forward fill")
print("- Preserves all 490,544 rows")
print()

# Column names to drop (the bad news columns)
...
# Step 2: Fetch daily-aggregated news from database
print("\nStep 2: Fetching daily-aggregated news from database...")

def fetch_daily_news(start_date, end_date):
    """Fetch news aggregated by date (not exact timestamp)"""
    db = SessionLocal()

    # FIXED: Use raw connection instead of read_sql to avoid bind issue
    from sqlalchemy import text

    # Base query (fetch all stocks at once)
    base_query = """
        SELECT
            stock_id,
    ...
    # Execute query once for all stocks (much faster!)
    db = SessionLocal()
    stock_news_all = pd.read_sql(base_query, db.bind(params={
        'start_date': start_date,
        'end_date': end_date
    }))

    # Then filter by our stock_ids
    stock_ids_str = ', '.join([f"'{sid}'" for sid in df['stock_id'].unique()])
    stock_query = f"""
    SELECT
            stock_id,
            date_trunc('day', "timestamp") as date,
            ...
            MAX("timestamp") as last_timestamp
        FROM stock_news
        WHERE stock_id IN ({stock_ids_str})
          AND "timestamp" >= :start_date AND "timestamp" <= :end_date
        GROUP BY stock_id, date_trunc('day', "timestamp")
        ORDER BY stock_id, date
    """

    # Execute query for each stock_id separately
    result_dfs = []
    unique_stocks = df['stock_id'].unique()

    for stock_id in tqdm(unique_stocks, desc="Fetching news"):
        # Add stock_id to WHERE clause
        stock_query = text(query + " AND stock_id = :stock_id")
        stock_news = pd.read_sql(stock_query, db.bind(params={
            'start_date': start_date,
            'end_date': end_date,
            'stock_id': stock_id
        }))
        ...
        if stock_news is not None and len(stock_news) > 0:
            result_dfs.append(stock_news.reset_index())
    ...
    # Combine all news
    news_df = pd.concat(result_dfs, ignore_index=True)
...
start_date = df['merge_date'].min()
end_date = df['merge_date'].max()

print(f"Date range: {start_date} to {end_date}")
print(f"  Fetching news for {df['stock_id'].nunique()} stocks...")

# Fetch news for all stocks at once
news_df = fetch_daily_news(start_date, end_date)

if news_df is None:
    print("❌ No news data retrieved!")
    sys.exit(1)

# Step 3: Merge on date (not exact timestamp)
print(f"\nStep 3: Merging news with features on date...")
print(f"  News records: {len(news_df)}")

# Prepare news for merge - use date for matching
news_df['date'] = pd.to_datetime(news_df['date']).dt.normalize()

# Also ensure timestamp is normalized (both sides)
df['timestamp'] = pd.to_datetime(df['timestamp']).dt.normalize()

# Merge on stock_id and date (NOT exact timestamp match!)
# This ensures we match all news for a day, not just exact timestamp
result = df.merge(
...
# Step 4: Fill missing news with 0 (neutral) - NO PROPAGATION
print(f"\nStep 4: Filling missing news with 0 (neutral sentiment)...")

# Define new news columns to create (20 columns)
...
# Fill with 0 (neutral) - no forward/backward propagation
for col in NEWS_COLUMNS_TO_CREATE:
    if col in result.columns:
        # Fill NaN with 0 (neutral sentiment)
        result[col] = result[col].fillna(0)
        # Also set data_available to 1 where we have any news
...
# Check how many have news vs no news
