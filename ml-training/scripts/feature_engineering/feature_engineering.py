...
- ADDED news sentiment features (20 new features) - Polygon API sentiment with multi-dimensional aggregation
...
SECRET WEAPONS:
1. SEC Form 4 Insider Trading (12 base features + 14 enhanced features)
...
2. News Sentiment from Polygon API (20 features)
Polygon's NLP sentiment analysis from article insights with sophisticated aggregation:
- Multiple rolling windows (1d to 30d) for short/medium/long-term sentiment
- Separate bullish/bearish signals (not just average!)
- Time-decay weighting (recent news = more important)
- Market hours alignment (after-hours → next day, prevents lookahead bias)
- Consensus/disagreement measurement
- Volume/intensity tracking
Multi-dimensional approach captures nuance that simple averaging misses!

Target: High-quality swing-trading features with raw numeric values + insider alpha + news sentiment + relative SPY
Output: ~159 features (OHLCV + indicators + swing + enhanced insider + news sentiment + relative SPY + minimal SPY context)
...
Changes on 2026-02-10 (News Sentiment):
- Added 20 news sentiment features from Polygon API
- Multi-dimensional aggregation (separate bullish/bearish, time-decay, consensus)
- Market hours alignment (after-hours → next day)
- Historical gap handling (2018-2019 set to 0)
"""
...
# Import news sentiment features
...
def main():
    ...
    for stock in tqdm(stocks_df.itertuples(), desc="Processing stocks"):
        ...
        try:
            ...
            # Add news sentiment features (Polygon API)
            df = NewsFeatures.add_news_features(df, stock_id, start_date, end_date)
    ...
    # Count news sentiment activity
    news_cols = [c for c in final_df.columns if 'news_' in c]
    if news_cols:
        news_available = final_df['news_data_available'].sum() if 'news_data_available' in final_df.columns else 0
        news_coverage = (news_available / len(final_df)) * 100 if len(final_df) > 0 else 0
        print(f"  📰 News Sentiment: {news_coverage:.1f}% coverage with {len(news_cols)} features")
